...
The runnable product is the Python package `matraix` plus **Harbor**.
Personas remain 1,290-dimension YAML records. Harbor `Job` / `Trial` still
execute Survey, Chatbot, Web, and App tasks. Playground and `matraix run`
are unchanged.

## AgentTwin Enterprise (this branch)

Additive control-plane direction on this fork — **not** a replacement for
MatrAIx, and not the H1 brand. Tenant-isolated organizations, shaped
synthetic workforces, experiment launch records, policy-gated model
routing, and evaluation that never treats simulated users as human
research. Code lives in `src/matraix/enterprise/`.

Phases **0–10** are on [PR #1](https://github.com/eli-labz/AgentTwin-8B/pull/1).
They are not a finished production platform. Benchmarks are a synthetic
sandbox probe, not a 1M soak. Compose/kustomize are references, not a
certified cluster.
...
HTTP: `/api/v1` (OpenAPI at `/docs` when the enterprise API is running).
Default store is in-memory; set `MATRIX_ENTERPRISE_STORE=sqlite` for a local
file. Docs: [docs/enterprise/](docs/enterprise/IMPLEMENTATION_ROADMAP.md).
...
Packaging and scale: [packaging.md](docs/enterprise/packaging.md),
[scale.md](docs/enterprise/scale.md).
...
> filesystem** (e.g. `~/AgentTwin-8B`, not `/mnt/c/…`, which is much slower) and
...
**AgentTwin Enterprise** (this branch, additive):
...
| [packaging.md](docs/enterprise/packaging.md) | Compose / kustomize references |
| [scale.md](docs/enterprise/scale.md) | 10 → 1M+ path (remote workers still stubs) |
