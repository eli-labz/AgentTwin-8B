...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="TENANT",
        help=(
            "Optional enterprise tenant id written only to the generated "
            ".meta.json sidecar. Does not change matraix run defaults."
        ),
    )
    parser.add_argument(
        "--n-concurrent-trials",
    ...
    if args.tenant_id:
        ...
        try:
            spec_tenant_id = TenantId(args.tenant_id).value
        except EnterpriseSchemaError as exc:
            parser.error(f"--tenant-id: {exc}")
    else:
        spec_tenant_id = None
    ...
    tenant_line = (
        f"# Enterprise tenant_id (sidecar metadata only): {meta['tenant_id']}\n"
